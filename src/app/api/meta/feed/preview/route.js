...
export async function GET(request) {
  try {
    ...
    const mapped = vehicles.map((vehicle) => {
      const mediaRows = mediaMap.get(vehicle.id) || [];
      const optionalNames = optionalsMap.get(vehicle.id) || [];

      const metaItem = mapVehicleToMetaItem({
        supabase,
        vehicle,
        mediaRows,
        optionalNames,
      ...
      return {
        ...
        source_media_count: mediaRows.length,
        source_optional_count: optionalNames.length,
        source_optional_names: optionalNames,
        meta_item: metaItem,
        included_in_feed: Boolean(metaItem),
        excluded_reason: !mediaRows.length ? "Nessuna immagine visibile" : null,
      };
    });

    const included = mapped.filter((item) => item.included_in_feed);
    const excluded = mapped.filter((item) => !item.included_in_feed);
    ...
    return Response.json(
      {
        ...
        totals: {
          ...
          included_in_feed: included.length,
          excluded_from_feed: excluded.length,
        },
        preview_limit: limit,
        preview_items: included.slice(0, limit),
        excluded_items: excluded.slice(0, limit),
      },
      {
        status: 200,
        headers: {
    ...
    console.error("META FEED PREVIEW ERROR:", error);

    return Response.json(
      {
        ok: false,
        error: error.message || "Errore generazione preview feed",
      },
      {
        status: 500,
        headers: {
